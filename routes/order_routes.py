...
async def create_order(user_schema: OrderSchema, session: Session = Depends(get_session)):
...
async def cancel_order(order_id: int, session: Session = Depends(get_session), user: User = Depends(verify_token)):
    ...
    if not user.admin and user.id != order.user:
        raise HTTPException(status_code=401, detail="Unfortunately, you do not have permission to make this change.") 
    order.status = "CANCELED"
    session.commit()
    return{
        "menssage": f"order number: {order.id} successfully canceled",
         "order": order
    }
...
async def add_item_order(order_id: int, item_schema: ItemSchema, session: Session = Depends(get_session), user: User = Depends(verify_token)):
    ...
    if not order:
        raise HTTPException(status_code=400, detail="that request does not exists")

    if not user.admin and order.id != order.user_id:
        raise HTTPException(status_code=401, detail="Unfortunately, you do not have permission to make this change")
    order_item = OrderItem(item_schema.quantity, item_schema.flavor, item_schema.size, item_schema.unit_price, order_id)
...
async def remove_item_order(order_item_id: int,
...
async def finish_order(order_id: int, session: Session = Depends(get_session), user: User = Depends(verify_token)):
    ...
    session.commit()
    return{
        "menssage": f"order number: {order.id} successfully finished",
         "order": order
    }

@order_router.get("/order/{order_id}")
async def visualize_order(order_id: int, session: Session = Depends(get_session), user: User = Depends(verify_token)):
    order = session.query(Order).filter(Order.id==order_id).first()
    if not order:
        raise HTTPException(status_code=400, detail="request not found")
    if not user.admin and user.id != order.user:
